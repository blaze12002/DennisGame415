...
Afirstperson415Projectile::Afirstperson415Projectile() 
{
	...
	//Creates a static mesh ball and assigns it to ballMesh variable
	ballMesh = CreateDefaultSubobject<UStaticMeshComponent>("Ball Mesh");
	
	// Set as root component
	RootComponent = CollisionComp;

	//Atatches the ball mesh to the collision component
...
//should determine a random color on projectile spawn and make projectile that color
...
void Afirstperson415Projectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (OtherActor != nullptr)
	{
		...
		//Checks to ensure colorP is valid
		if (colorP)
		{
			//Spawns particles and destroys the ball theoretically. none of this currently triggers, presumably because colorP is null.
		...
		//spawns and creates decal at location
		auto Decal = UGameplayStatics::SpawnDecalAtLocation(GetWorld(), baseMat, FVector(UKismetMathLibrary::RandomFloatInRange(20.f, 40.f)), Hit.Location, Hit.Normal.Rotation(), 0.f);
		auto MatInstance = Decal->CreateDynamicMaterialInstance();

		//sets color and shape of decal
